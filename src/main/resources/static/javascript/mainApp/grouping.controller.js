...
(() => {
  ...
  function GroupingJsController(
    ...
    $scope.displayGrouping = (currentPage, index) => {
      $scope.selectedGrouping =
        $scope.pagedItemsGroupings[Number(currentPage)][Number(index)];
    ...
    $scope.displayOwnerGroupingInNewTab = (basePath, groupingName) => {
      sessionStorage.setItem(
        "selectedOwnerGrouping",
        JSON.stringify({ path: basePath, name: groupingName })
      );
        ...
        groupingsService.getGrouping(
          ...
          (res) => {
            if (res.paginationComplete) {
              ...
              res.groupingBasis.groupPath.slice(0, -6) ===
              $scope.selectedGrouping.path
            ) {
              let putGroupMembers = combineGroupMembers;
              if (currentPage === 1) {
                putGroupMembers = setGroupMembers;
              }
              $scope.groupingBasis = putGroupMembers(
                res.groupingBasis.members,
                $scope.groupingBasis
              );
              $scope.filter(
                $scope.groupingBasis,
              ...
              $scope.groupingInclude = putGroupMembers(
                res.groupingInclude.members,
                $scope.groupingInclude
              );
              ...
              $scope.groupingExclude = putGroupMembers(
                res.groupingExclude.members,
                $scope.groupingExclude
              );
              ...
              $scope.groupingMembers = putGroupMembers(
                res.allMembers.members,
                $scope.groupingMembers
              );
              $scope.filter(
                $scope.groupingMembers,
    ...
    $scope.fetchOwners = (groupPath) => {
      return new Promise((resolve) => {
        groupingsService.groupingOwners(
          groupPath,
          (res) => {
    ...
    $scope.getGroupingDescription = (groupPath) => {
      return new Promise((resolve) =>
        groupingsService.getGroupingDescription(
          groupPath,
          (res) => {
    ...
    $scope.getGroupingSyncDest = (groupPath) => {
      return new Promise((resolve) =>
        groupingsService.getGroupingSyncDest(
          groupPath,
          (res) => {
    ...
    $scope.getGroupingOptAttributes = (groupPath) => {
      return new Promise((resolve) =>
        groupingsService.getGroupingOptAttributes(
          groupPath,
          (res) => {
    ...
    $scope.addInInclude = (group) => {
      group.forEach((member) => {
        const memberUhUuid = member.uhUuid;
        member.inInclude = _.some($scope.groupingInclude, {
          uhUuid: memberUhUuid,
        })
          ? "Yes"
          : "No";
    ...
    $scope.addInExclude = (group) => {
      group.forEach((member) => {
        const memberUhUuid = member.uhUuid;
        member.inExclude = _.some($scope.groupingExclude, {
          uhUuid: memberUhUuid,
        })
          ? "Yes"
          : "No";
     ...
     */
    $scope.descriptionLengthWarning = () =>
      String($scope.modelDescription).length > $scope.maxDescriptionLength - 1;
    ...
    $scope.saveDescription = () => {
      if (
        $scope.groupingDescription.localeCompare($scope.modelDescription) === 0
      ) {
        return $scope.cancelDescriptionEdit();
      }
      $scope.groupingDescription =
        $scope.modelDescription.length > 0
          ? $scope.modelDescription
          : $scope.noDescriptionMessage;
      groupingsService.updateDescription(
        $scope.groupingDescription,
        $scope.selectedGrouping.path,
        () => {
          $scope.descriptionForm = !$scope.descriptionForm;
    ...
    $scope.descriptionDisplay = () => {
      ...
      return $scope.groupingDescription.length > 0
        ? $scope.groupingDescription
        : $scope.noDescriptionMessage;
    };
    ...
    $scope.resetFields = () => {
      ...
      $scope.pageSelected = false;
      $scope.membersInCheckboxList = _.mapValues(
        $scope.membersInCheckboxList,
        () => false
      );
    };
    ...
    $scope.displayImportModal = (listName) => {
      ...
      $scope.importModalInstance.result.catch(() => {
    ...
    $scope.readTextFile = (inputFile) => {
      ...
      reader.onload = (e) => {
        ...
        if (inputFile.type === "text/csv") {
          ...
          const indexOfUhNumber = firstRow.findIndex((header) =>
            header.includes(Message.Csv.UUID_COLUMN_HEADER)
          );
          ...
          const UHNumbersInFile = namesInFile
            .map((row) => row.split(",")[Number(indexOfUhNumber)])
            .slice(1, -1);
    ...
    $scope.verifyImportFileType = (inputFile) => {
      ...
      return (
        inputFile.type.toLowerCase() === "text/plain" ||
        inputFile.type.toLowerCase() === "text/csv"
      );
    };
    ...
    $scope.existsInList = (listName, members) => {
      ...
      const membersInList = members.filter(
        (member) =>
          _.some(currentPage, { uhUuid: member }) ||
          _.some(currentPage, { uid: member }) ||
          _.some(currentPage, { name: $scope.groupingName })
      );
      ...
      if (listName === "Include" || listName === "Exclude") {
        if ($scope.manageMembers.includes(":")) {
          $scope.displayDynamicModal(
            ...
            Message.Body.ADD_GROUP_PATH_ERROR
          );
          clearMemberInput();
          return;
        } else {
        ...
        if ($scope.manageMembers.includes(":")) {
          if ($scope.selectedGrouping.path === $scope.manageMembers) {
            ...
            $scope.displayDynamicModal(
              ...
              Message.Body.ADD_CURRENT_PATH_ERROR
            );
            clearMemberInput();
            return;
            ...
            $scope.displayDynamicModal(
              Message.Title.INVALID_MULTI_ADD,
              Message.Body.INVALID_MULTI_ADD
            );
            clearMemberInput();
            return;
          } else {
    ...
    $scope.addMembers = (listName, uhIdentifiers) => {
      ...
      uhIdentifiers =
        uhIdentifiers ?? $scope.parseAddRemoveInputStr($scope.manageMembers);
      ...
      uhIdentifiers = uhIdentifiers.filter(
        (member) => !$scope.membersInList.includes(member)
      );
    ...
    $scope.displayAddModal = (options) => {
      ...
      $scope.hasDeptAccount = $scope.checkForDeptAccount(
        membersAttributesResults
      );
      ...
      const templateUrl = $scope.isMultiAdd
        ? "modal/multiAddModal"
        : $scope.addModalURL;
      ...
      $scope.addModalInstance.result.then(
        async () => {
          $scope.waitingForImportResponse = true; // Small spinner on
          const groupingPath = $scope.selectedGrouping.path;
          if ($scope.listName === "Include") {
            await groupingsService.addIncludeMembers(
              uhIdentifiers,
              groupingPath,
              handleSuccessfulAdd,
            ...
            await groupingsService.addExcludeMembers(
              uhIdentifiers,
              groupingPath,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest,
            ...
            await groupingsService.addOwnerships(
              groupingPath,
              uhIdentifiers,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest
            ...
            await groupingsService.addGroupPathOwnerships(
              groupingPath,
              uhIdentifiers,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest
            ...
            await groupingsService.addAdmin(
              uhIdentifiers,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest
            );
    ...
    $scope.displayImportConfirmationModal = (listName, membersToAdd) => {
      ...
      const groupingPath = $scope.selectedGrouping.path;
      $scope.importConfirmationModalInstance.result.then(
        async () => {
          $scope.waitingForImportResponse = true; // Small spinner on
          if ($scope.listName === "Include") {
            await groupingsService.addIncludeMembersAsync(
              membersToAdd,
              groupingPath,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest,
            ...
            await groupingsService.addExcludeMembersAsync(
              membersToAdd,
              groupingPath,
              handleSuccessfulAdd,
              handleUnsuccessfulRequest,
    ...
    $scope.returnMemberObject = (memberIdentifier, listName) => {
      ...
      if (/[0-9]{8}/.test(memberIdentifier)) {
        memberToReturn = _.find(
          currentPage,
          (member) => member.uhUuid === memberIdentifier
        );
      } else {
        memberToReturn = _.find(
          currentPage,
          (member) => member.uid === memberIdentifier
        );
      }
      if ($scope.isOwnerGrouping) {
        memberToReturn = _.find(
          currentPage,
          (member) => member.name === $scope.groupingName
        );
      }
    ...
    $scope.fetchMemberProperties = (members, listName) => {
      ...
      $scope.multiRemoveResults = _.uniqBy(
        membersInList,
        (member) => member.uhUuid
      );
    ...
    $scope.removeMembers = (listName) => {
      ...
      $scope.membersToModify = _.isEmpty($scope.manageMembers)
        ? $scope.extractSelectedUsersFromCheckboxes(
            $scope.membersInCheckboxList
          )
        : $scope.manageMembers;
      ...
      if (!$scope.fetchMemberProperties(uhIdentifiers, listName)) {
        $scope.displayDynamicModal(
          Message.Title.REMOVE_INPUT_ERROR,
          Message.Body.REMOVE_INPUT_ERROR
        );
        $scope.membersNotInList = "";
        return;
      }
      ...
      if (
        listName === "owners" &&
        $scope.multiRemoveResults.length === $scope.groupingOwners.length
      ) {
      ...
      uhIdentifiers = uhIdentifiers.filter(
        (member) => !$scope.membersNotInList.includes(member)
      );
    ...
    const handleSuccessfulRemove = () => {
      ...
      if ($scope.isMultiRemove) {
        ...
        $scope.displayDynamicModal(
          ...
          Message.Body.REMOVE_GROUP_PATH.with(
            $scope.groupingName,
            $scope.listName
          )
        );
    ...
    const handleOwnerRemove = () => {
      ...
      if (
        !(
          ...
          $scope.membersToRemove.includes($scope.currentUser.uid)
        ) ||
        !_.isUndefined($scope.adminsList)
      ) {
    ...
    $scope.displayRemoveModal = (options) => {
      ...
      if (!$scope.isOwnerGrouping) {
        const memberObject = $scope.returnMemberObject(
          $scope.membersToRemove[0],
          $scope.listName
        );
      ...
      const templateUrl = $scope.isMultiRemove
        ? "modal/multiRemoveModal"
        : $scope.removeModalURL;
      ...
      $scope.removeModalInstance.result.then(
        () => {
          ...
          const groupingPath = $scope.selectedGrouping.path;
          if ($scope.listName === "Include") {
            groupingsService.removeIncludeMembers(
              groupingPath,
              $scope.membersToRemove,
              handleSuccessfulRemove,
              handleUnsuccessfulRequest
            ...
            groupingsService.removeExcludeMembers(
              groupingPath,
              $scope.membersToRemove,
              handleSuccessfulRemove,
              handleUnsuccessfulRequest
            ...
            groupingsService.removeOwnerships(
              groupingPath,
              $scope.membersToRemove,
              handleOwnerRemove,
              handleUnsuccessfulRequest
            ...
            groupingsService.removeGroupPathOwnerships(
              groupingPath,
              $scope.membersToRemove,
              handleOwnerRemove,
              handleUnsuccessfulRequest
    ...
    $scope.removeMemberWithTrashcan = (listName, currentPage, index) => {
      ...
      if (listName === "Include") {
        memberToRemove =
          $scope.pagedItemsInclude[Number(currentPage)][Number(index)];
      } else if (listName === "Exclude") {
        memberToRemove =
          $scope.pagedItemsExclude[Number(currentPage)][Number(index)];
      }
    ...
    $scope.removeOwnerWithTrashcan = (currentPage, index) => {
      const ownerToRemove =
        $scope.pagedItemsOwners[Number(currentPage)][Number(index)];
    ...
    $scope.initResetGroup = async () => {
      ...
      if (resetInclude) {
        if ($scope.groupingInclude.length <= Threshold.RESET_GROUP) {
          await resetGroup(
            groupingsService.resetIncludeGroup,
            groupingPath,
            resetIncludeResult
          );
        } else {
          await resetGroup(
            groupingsService.resetIncludeGroupAsync,
            groupingPath,
            resetIncludeResult
          );
        }
      ...
      if (resetExclude) {
        if ($scope.groupingExclude.length <= Threshold.RESET_GROUP) {
          await resetGroup(
            groupingsService.resetExcludeGroup,
            groupingPath,
            resetExcludeResult
          );
        } else {
          await resetGroup(
            groupingsService.resetExcludeGroupAsync,
            groupingPath,
            resetExcludeResult
          );
        }
      ...
      $scope.handleGroupingReset(
        groupingPath,
        resetIncludeResult.pop(),
        resetExcludeResult.pop()
      );
    };
    ...
    $scope.displayUnsuccessfulGroupResetModal = (message) => {
      $scope.displayDynamicModal(
        Message.ResetGroupError.ResetGroupErrorMessages.TITLE,
        message
      );
    };
    ...
    $scope.handleGroupingReset = (
      ...
      let wasError = (result) => {
        return (
          typeof result !== "undefined" && typeof result.status !== "undefined"
        );
      };
      let wasSuccess = (groupingPath, resetResult) => {
        return (
          typeof resetResult !== "undefined" &&
          resetResult.resultCode === "SUCCESS" &&
          resetResult.groupPath.includes(groupingPath)
        );
      };
      ...
      if (results.includeFailure || results.excludeFailure) {
        for (
          let i = 0;
          i < Message.ResetGroupError.ResetGroupErrorMessageMap.length;
          i++
        ) {
          let object =
            Message.ResetGroupError.ResetGroupErrorMessageMap[Number(i)];
          ...
          if (Utility.compareObjects(object, results)) {
            ...
            let message =
              Message.ResetGroupError.ResetGroupErrorMessages.Body[Number(i)];
            $scope.displayUnsuccessfulGroupResetModal(message);
            return;
          }
        }
    ...
    $scope.updateAllowOptOut = () => {
      ...
      groupingsService.updateOptOut(
        groupingPath,
        allowOptOut,
        handleSuccessfulPreferenceToggle,
        handleUnsuccessfulRequest
      );
    };
    ...
    $scope.getSyncDestValueInArray = (syncDestName) => {
      return $scope.syncDestArray.find(
        (element) => element.name === syncDestName
      ).synced;
    };
    ...
    $scope.setSyncDestInArray = (syncDestName, syncDestvalue) => {
      $scope.syncDestArray.find(
        (element) => element.name === syncDestName
      ).synced = syncDestvalue;
    };
    ...
    $scope.updateSingleSyncDest = (syncDestName) => {
      ...
      groupingsService.updateSyncDest(
        groupingPath,
        syncDestName,
        syncDestOn,
        handleSuccessfulPreferenceToggle,
    ...
    $scope.submitPreferences = () => {
      ...
      if (optInChanged) {
        modalContent.push(
          `${$scope.allowOptIn ? "(Enable)" : "(Disable)"} Opt-in preference`
        );
      }
      if (optOutChanged) {
        modalContent.push(
    ...
    $scope.anySyncDestChanged = () => {
      return $scope.syncDestArray.some(
        (syncDest) => syncDest.initialSynced !== syncDest.synced
      );
    };
    ...
    $scope.submitSelectedDestinations = () => {
      ...
      const changedDestinations = $scope.syncDestArray.filter(
        (syncDest) => syncDest.synced !== syncDest.initialSynced
      );

      // Build the modal content only for changed destinations
      changedDestinations.forEach((syncDest) => {
        modalContent.push(
          `${syncDest.synced ? "(Enable)" : "(Disable)"} ${
      ...
      if (modalContent.length !== 0) {
        ...
        $scope.syncDestInstance.result.then(
          () => {
            ...
            $scope.displayDynamicModal(
              "Sync Confirmation",
              "The selected destinations have been synced successfully."
            );
          },
          () => {}
        );
    ...
    $scope.transferMembersFromPageToCheckboxObject = (currentPage) => {
      currentPage.forEach(
        (member) => ($scope.membersInCheckboxList[member.uhUuid] = false)
      );
    };
    ...
    $scope.toggleSingleCheckbox = (group, member) => {
      $scope.assignListToGroup(group);
      $scope.membersInCheckboxList[member.uhUuid] =
        !$scope.membersInCheckboxList[member.uhUuid];
      $scope.pageSelected = $scope.checkMainSelectAllCheckbox(
        $scope.groupingMembers
      );
      $scope.paginationPageChange = false;
    };
    ...
    $scope.isGroupingEmpty = () => {
      return (
        $scope.groupingMembers.length === 0 &&
  ...
  function OptPreferenceModalController(
    $scope,
    $uibModalInstance,
    isSingular,
    preferenceChanges,
    ...
    $scope.optPreferenceConfirmationMessage =
      Message.OptPreferencesModal.confirmationMessage(isSingular);
  ...
  function SyncDestModalController(
    $scope,
    $uibModalInstance,
    isSingular,
    syncDestDescription,
    ...
    $scope.syncDestConfirmationMessage =
      Message.SyncDestModal.confirmationMessage(isSingular);
  ...
  UHGroupingsApp.controller(
    "OptPreferenceModalController",
    OptPreferenceModalController
  );
