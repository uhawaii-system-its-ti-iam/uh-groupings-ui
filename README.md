...
##### Out-of-the-box (OOTB) local development
OOTB is a self-contained local environment for UH Groupings. It uses mock data instead of CAS, LDAP, or Grouper, so the API and UI can run without UH infrastructure.

OOTB lives on the `ootb` branch, not `main`. See the OOTB READMEs for setup and run instructions:
